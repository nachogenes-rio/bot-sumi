...
function cargarExcel() {
  try {
    ...
    // Hoja: Sumi UF — headers en fila 7 (índice 6)
    ...
    process.exit(1); // Si falla el Excel, Railway lo muestra como caído — mejor morir rápido
  }
}
...
// "CL sin Vender" usa formato "MOLINA TOMAS" (sin comas)
...
// "Avance xVend." puede tener formato variable
  ...
  return JSON.stringify({
    ...
    clientes: clientes.map(c => ({
      ...
      hamburguesas: {
        ...
        mediana:    !!c.mediana,
        clasica:    !!c.clasica,
        parrillera: !!c.parrillera,
      },
      salchichas: { s230: !!c.s230, s190: !!c.s190, x12: !!c.sx12 },
      rebozados:  { mila_o_nugg: !!c.mila },
      faltantes:  (c.faltante && c.faltante !== 'nan')
                    ? c.faltante.replace(/\+/g, ', ')
                    : null,
    })),
    sin_vender: sinVender.map(c => ({
      cod: c.cod, nombre: c.razon, barrio: c.loc, dia: c.dia,
    })),
    avance: avance ? {
      ...
      hamburguesas: {
        vendido:  fmtKg(avance.av_hamb),
        objetivo: fmtKg(avance.obj_hamb),
        pct:      fmtPct(avance.pct_hamb),
      },
      salchichas: {
        vendido:  fmtKg(avance.av_salch),
        objetivo: fmtKg(avance.obj_salch),
        pct:      fmtPct(avance.pct_salch),
      },
      rebozados: {
        vendido:  fmtKg(avance.av_rebo),
        objetivo: fmtKg(avance.obj_rebo),
        pct:      fmtPct(avance.pct_rebo),
      },
    } : null,
  });
}
...
async function preguntarIA(pregunta, contexto, historial) {
  ...
  if (!apiKey) {
    ...
    return '⚠️ Sin clave de IA configurada. Contactá al administrador.';
  }

  const system = `Sos el asistente de ventas de SuMi Distribuidora. Respondés preguntas de vendedores sobre sus clientes y su rendimiento.
...
- Usá emojis para que sea más legible en WhatsApp
- Al listar clientes, máximo 10 por mensaje; si hay más, indicá el total
...
${contexto}`;
  ...
  try {
    ...
    if (data.error) {
      ...
      return '⚠️ Error con la IA. Intentá de nuevo.';
    }
    if (!data.content?.[0]?.text) {
      console.error('❌ Respuesta vacía:', JSON.stringify(data));
      return '⚠️ Respuesta vacía. Intentá de nuevo.';
    }
...
app.post('/webhook', async (req, res) => {
  ...
  // Esperando usuario
    ...
    if (USUARIOS[user]) {
      ...
      respuesta = '❌ Usuario no encontrado.\n\nUsá tu apellido en minúsculas, ej: *molina*, *aquino*, *lopez*';
    }
  }
  ...
  // Esperando contraseña
    ...
    if (texto === u.pass) {                       // contraseña es case-sensitive
      ...
      respuesta = `✅ *¡Bienvenido, ${u.nombre}!*\n\n📊 Tenés *${clis} clientes* en tu cartera y *${sinV} sin comprar* esta semana.\n\n🤖 Preguntame lo que quieras:\n\n• _¿Cómo voy contra el objetivo?_\n• _¿Qué clientes no compraron esta semana?_\n• _¿A quién le falta hamburguesa finita?_\n• _Dame info de [nombre del cliente]_\n• _¿Qué clientes tengo los jueves?_\n\n_(Escribí *salir* para cerrar sesión)_`;
    } else {
      s.estado  = 'inicio';
      s.usuario = null;
  ...
  console.log(`📤 [${from}] "${respuesta.slice(0, 80)}..."`);
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 Bot SuMi corriendo en puerto ${PORT}`);
});
