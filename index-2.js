...
const app = express();
// ⚠️ CRÍTICO: necesitamos ambos parsers — Twilio manda urlencoded pero a veces JSON
...
function cargarExcel() {
  try {
    ...
    console.log(`✅ Excel cargado: ${DB.clientes.length} clientes, ${DB.sin_vender.length} sin vender, ${DB.avance.length} en avance`);
  } catch (e) {
...
// ─── HELPERS ──────────────────────────────────────────────────
function getClientesPorVendedor(v) {
  return DB.clientes.filter(c => c.vendedor === v);
}

// Busca en "CL sin Vender" (formato "MOLINA TOMAS" sin comas)
function getSinVender(v) {
  const partes   = v.split(',').map(p => p.trim().toUpperCase());
  const apellido = partes[0];
  const nombre   = partes[1] || '';
  return DB.sin_vender.filter(c => {
    const vUp = c.vendedor.toUpperCase();
    return vUp.includes(apellido) && (!nombre || vUp.includes(nombre));
  });
}

// Busca en "Avance xVend." (formato puede variar)
function getAvance(v) {
  const partes   = v.split(',').map(p => p.trim().toUpperCase());
  const apellido = partes[0];
  const nombre   = partes[1] || '';
  return DB.avance.find(a => {
    const aUp = a.vendedor.toUpperCase();
    return aUp.includes(apellido) && (!nombre || aUp.includes(nombre));
  }) || null;
}
...
async function preguntarClaude(pregunta, contexto, historial) {
  ...
  if (!ANTHROPIC_API_KEY) {
    ...
    return '⚠️ No está configurada la clave de la IA. Contactá al administrador.';
  }
...
${contexto}`;
  ...
  try {
    ...
    if (data.error) {
      ...
      return '⚠️ Error al consultar la IA. Intentá de nuevo.';
    }
    if (!data.content || !data.content[0]) {
      console.error('❌ Claude respuesta vacía:', JSON.stringify(data));
      return '⚠️ Respuesta vacía de la IA. Intentá de nuevo.';
    }

    return data.content[0].text;
  } catch (e) {
    console.error('❌ Fetch error llamando a Claude:', e.message);
...
app.get('/', (req, res) => res.send('✅ Bot SuMi activo'));
...
app.post('/webhook', async (req, res) => {
  ...
  if (!from) {
    console.warn('⚠️ Webhook recibido sin campo From');
  ...
  console.log(`📩 [${from}] estado=${s.estado} msg="${texto}"`);

  let respuesta = '';

  // ── Reset / Saludo inicial ──
  if (s.estado === 'inicio' || msg === 'hola' || msg === 'start' || msg === 'hi') {
    s.estado    = 'esperando_usuario';
    s.historial = [];
    respuesta   = '👋 Bienvenido al sistema de consultas *SuMi Distribuidora*.\n\nIngresá tu *usuario*:';
  }

  // ── Cerrar sesión (desde cualquier estado) ──
  ...
  // ── Esperando usuario ──
    ...
    if (USUARIOS[user]) {
      ...
      respuesta = '❌ Usuario no encontrado. Intentá de nuevo\n\n_(Escribí tu usuario en minúsculas, ej: *molina*)_';
    }
  }
...
app.listen(PORT, () => console.log(`🚀 Bot SuMi activo en puerto ${PORT}`));
